...
public class ThermostatRenderer extends QueueFeeder<ZoneSnapshot> implements DataSink<ThermostatSignal>, ProtobufRenderer<DataSample<ThermostatSignal>> {
    ...
    public void consume(DataSample<ThermostatSignal> signal) {
        ...
        ZoneSnapshot.Builder builder = ZoneSnapshot.newBuilder();
        
        builder
            .setTimestamp(signal.timestamp)
            .setName(name)
            .setMode(convert(mode))
            .setState(convert(state))
            .setSignal(thermostatSignal)
        ...
        // protobuf doesn't allow setting even optional fields to null
        if (periodName != null) {
            
            builder.setPeriodName(periodName);
        }
        
        builder
            .setDeviationSetpoint(deviation.setpoint)
            .setDeviationEnabled(deviation.enabled)
            .setDeviationVoting(deviation.voting);
    ...
    private net.sf.dz3.view.http.v3.ZoneSnapshotList.HvacMode convert(HvacMode mode) {

        return net.sf.dz3.view.http.v3.ZoneSnapshotList.HvacMode.valueOf(mode.mode);
    }

    private net.sf.dz3.view.http.v3.ZoneSnapshotList.ZoneState convert(ZoneState state) {

        switch (state) {

        case ERROR:

            return net.sf.dz3.view.http.v3.ZoneSnapshotList.ZoneState.ERROR;

        case OFF:

            return net.sf.dz3.view.http.v3.ZoneSnapshotList.ZoneState.ZONE_OFF;

        case CALLING:

            return net.sf.dz3.view.http.v3.ZoneSnapshotList.ZoneState.CALLING;

        case HAPPY:

            return net.sf.dz3.view.http.v3.ZoneSnapshotList.ZoneState.HAPPY;

        default:

            throw new IllegalStateException("Did we add a new value to ZoneState enum?");
        }
    }
